"""
Build Contract NLI hypergraphs with batch support.

Supported input schema:
	data/nli/contract_nli.split.jsonl
...
Output layout:
	data/debug/contract_nli/split/<instance_id>/
    - query_hypergraph.pkl
    - data_hypergraph0.pkl
    - metadata.json
  data/debug/contract_nli/cleaned/summary.json
  data/debug/contract_nli/cleaned/failed.json

Examples:
  python tests/tasks/build_contract_nli.py --use-gpu-batch --batch-size 32
  python tests/tasks/build_contract_nli.py --force-rebuild
"""
...
def _normalize_item(raw: dict, row_idx: int) -> dict | None:
	"""Normalize Contract NLI rows into a build-ready structure."""
...
def _load_contract_nli_items(path_str: str) -> list[dict]:
	"""Load Contract NLI rows from a JSONL file."""
	path = Path(path_str)
	if not path.exists():
...
def batch_text_to_hypergraph(
	...
	"""Convert texts to hypergraphs with nlp.pipe in strict batch mode."""
