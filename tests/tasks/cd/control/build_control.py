"""
Build ConTRoL hypergraphs with batch support.

Supported input schema:
  data/nli/ConTRoL50.jsonl
...
Output layout:
  data/debug/control/sample80/<instance_id>/
    - query_hypergraph.pkl
    - data_hypergraph0.pkl
    - metadata.json
  data/debug/control/sample80/summary.json
  data/debug/control/sample80/failed.json

Examples:
  python tests/tasks/build_control.py --use-gpu-batch --batch-size 32
  python tests/tasks/build_control.py --force-rebuild
"""
...
target_dir = "data/debug/control/sample80"
dataset_path = "data/nli/ConTRoL50.jsonl"
...
def _normalize_item(raw: dict, row_idx: int) -> dict | None:
	"""Normalize ConTRoL rows into a build-ready structure."""
	...
	return {
		...
		"question": hypothesis,
		"answer": label,
		"context_docs": [premise],
		"source_file": "ConTRoL50.jsonl",
...
def _load_control_items(path_str: str) -> list[dict]:
	"""Load ConTRoL rows from a JSONL file."""
	path = Path(path_str)
	if not path.exists():
...
def _build_all_hypergraphs_single(
	...
	data = _load_control_items(dataset_path)
	...
	for item in tqdm(data, desc="Building control hypergraphs (single)"):
		try:
			...
			metadata = {
				...
				"dataset": item.get("dataset", "control"),
...
def _print_summary(summary: dict) -> None:
	print("\n" + "=" * 60)
	print("ConTRoL hypergraph build finished")
	print("=" * 60)
...
def main() -> None:
	parser = ArgumentParser(description="Build and store ConTRoL hypergraphs")
	...
	parser.add_argument("--batch-size", type=int, default=1024, help="Batch size for GPU batch mode")
