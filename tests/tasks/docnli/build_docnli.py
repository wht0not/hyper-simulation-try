"""
Build DocNLI hypergraphs with batch support.

Supported input schema:
  data/nli/docnli.jsonl
...
Output layout:
  data/debug/docnli/sample/<instance_id>/
    - query_hypergraph.pkl
    - data_hypergraph0.pkl
    - metadata.json
  data/debug/docnli/sample/summary.json
  data/debug/docnli/sample/failed.json

Examples:
  python tests/tasks/build_docnli.py --use-gpu-batch --batch-size 32
  python tests/tasks/build_docnli.py --force-rebuild
"""
...
target_dir = "data/debug/docnli/sample50"
dataset_path = "data/nli/docnli_50.jsonl"
...
def _normalize_item(raw: dict, row_idx: int) -> dict | None:
	"""Normalize DocNLI rows into a build-ready structure."""
	...
	return {
		...
		"source_file": "docnli.jsonl",
		"dataset": raw.get("dataset", "docnli"),
...
def _load_docnli_items(path_str: str) -> list[dict]:
	"""Load DocNLI rows from a JSONL file."""
	path = Path(path_str)
	if not path.exists():
...
def _build_all_hypergraphs_single(
	...
	data = _load_docnli_items(dataset_path)
	...
	for item in tqdm(data, desc="Building docnli hypergraphs (single)"):
		try:
			...
			metadata = {
				...
				"dataset": item.get("dataset", "docnli"),
...
def _print_summary(summary: dict) -> None:
	print("\n" + "=" * 60)
	print("DocNLI hypergraph build finished")
	print("=" * 60)
...
def main() -> None:
	parser = ArgumentParser(description="Build and store DocNLI hypergraphs")
