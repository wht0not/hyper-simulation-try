...
DEFAULT_OUTPUT_DIR = "data/baseline/docnli"
DEFAULT_DATASET_PATHS = ["data/nli/docnli_50.jsonl"]
EVAL_VERSION = "docnli_entailment_binary_v2"
...
def _load_docnli_items(dataset_paths: list[str], limit: int | None = None) -> list[dict[str, Any]]:
	...
	for one_path in dataset_paths:
		...
		if not path.exists():
			continue
		for item in load_task_dataset(task="docnli", dataset_path=str(path)):
...
def run_docnli_baseline(
	...
	# Baseline only uses contradoc; no LLM classification call.
	...
	method = "contradoc"
	out_file = out_root / f"docnli_{method}.json"
	existing_result_map = _load_existing_results_map(out_file)

	dataset_rows = _load_docnli_items(dataset_paths=dataset_paths, limit=limit)
	if not dataset_rows:
		global_summary["results"][method] = {"status": "skipped", "reason": "empty_or_missing"}
		_safe_write_json(out_root / "docnli_baseline_summary.json", global_summary)
	...
	for item in pbar_exec:
		...
		# Contradoc-only baseline for entailment-positive setting.
...
def main() -> None:
	parser = argparse.ArgumentParser(description="DocNLI baseline (contradoc-only, no LLM classification)")
