...
from hyper_simulation.question_answer.decompose import (
	decompose_question_with_nums_batch,
	mark_vertex_ids_for_subquestions_batch,
)
...
DEFAULT_OUTPUT_PATH = "data/debug/multihop/decompose_with_nums.json"
...
def _infer_num_subquestions(item: dict[str, Any]) -> int:
...
def run_batch_decompose_multihop(
	...
	for instance_dir in instance_dirs:
		...
		num_subquestions = _infer_num_subquestions(item=item)
		if num_subquestions <= 0:
			samples.append(
				{
					"instance_id": instance_dir.name,
					"status": "skipped",
					"reason": "evidence_list_missing_or_empty",
				}
			)
			continue
		samples.append(
	...
	pbar = tqdm(total=len(pending), desc="Decompose multihop with nums", unit="inst")
	for start in range(0, len(pending), batch_size):
		chunk = pending[start : start + batch_size]
		questions = [s["question"] for s in chunk]
		nums = [s["num_subquestions"] for s in chunk]
		queries = [s["query_hg"] for s in chunk]

		generated_subs_batch = decompose_question_with_nums_batch(questions=questions, nums=nums)
		batch_outputs = mark_vertex_ids_for_subquestions_batch(
			questions=questions,
			subs_batch=generated_subs_batch,
			queries=queries,
		)

		for sample, generated_subs, output in zip(chunk, generated_subs_batch, batch_outputs):
			...
			instance_result = {
				...
				"num_subquestions": sample["num_subquestions"],
				"generated_subquestions": generated_subs,
			...
			if show_live_details:
				...
				tqdm.write(f"[num_subquestions] {sample['num_subquestions']}")
...
def main() -> None:
	parser = argparse.ArgumentParser(
		description=(
			"Batch decompose MultiHop questions without preset subquestions: "
			"generate subquestions by number, then mark query-vertex ids."
		)
	)
	...
	report = run_batch_decompose_multihop(
