...
DEFAULT_INSTANCES_ROOT = "data/debug/contract_nli/sample65"
DEFAULT_DATASET_PATH = "data/nli/contract_nli_split_sample65.jsonl"
DEFAULT_OUTPUT_PATH = "data/debug/contract_nli/contract_nli.json"


CONTRACT_NLI_PROMPT = """### Task:
Given a contract premise and a hypothesis, determine whether the premise entails the hypothesis.
...
def _extract_contract_context_text(item: dict[str, Any]) -> str:
...
def _load_instance_graphs(instance_dir: Path, item: dict[str, Any]) -> tuple[LocalHypergraph | None, list[dict[str, Any]]]:
	...
	context_text = _extract_contract_context_text(item)
...
def run_contract_nli_multihop_evaluation(
	...
	dataset_index = load_dataset_index(task="contract_nli", dataset_path=dataset_path)
	...
	for instance_dir in tqdm(instance_dirs, desc="Contract NLI", unit="inst"):
		...
		prompt = CONTRACT_NLI_PROMPT.format(
...
def main() -> None:
	parser = argparse.ArgumentParser(description="Contract NLI with hyper-simulation hints")
	...
	run_contract_nli_multihop_evaluation(
