"""
Build Econ hypergraphs with batch support.

Supported input schema:
  data/nli/econ_qa.jsonl
...
Output layout:
  data/debug/econ/sample/<instance_id>/
    - query_hypergraph.pkl
    - data_hypergraph0.pkl
    - metadata.json
  data/debug/econ/sample/summary.json
  data/debug/econ/sample/failed.json

Examples:
  python tests/tasks/build_econ.py --use-gpu-batch --batch-size 32
  python tests/tasks/build_econ.py --force-rebuild
"""
...
target_dir = "data/debug/econ/sample"
dataset_path = "data/nli/econ_qa.jsonl"
...
def _normalize_item(raw: dict, row_idx: int) -> dict | None:
	"""Normalize Econ rows into a build-ready structure."""
	...
	return {
		...
		"source_file": "econ_qa.jsonl",
		"dataset": raw.get("dataset", "econ"),
...
def _load_econ_items(path_str: str) -> list[dict]:
	"""Load Econ rows from a JSONL file."""
	path = Path(path_str)
	if not path.exists():
...
def _build_all_hypergraphs_single(
	...
	data = _load_econ_items(dataset_path)
	...
	for item in tqdm(data, desc="Building econ hypergraphs (single)"):
		try:
...
def _print_summary(summary: dict) -> None:
	print("\n" + "=" * 60)
	print("Econ hypergraph build finished")
	print("=" * 60)
...
def main() -> None:
	parser = ArgumentParser(description="Build and store Econ hypergraphs")
