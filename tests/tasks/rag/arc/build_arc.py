"""
Build ARC Challenge-test hypergraphs.

This builder is challenge-only by default and supports two modes:
1) single-text mode
2) GPU batch mode

When retrieval context is missing, it falls back to one data document that
contains question + options, so each instance has at least one data graph.
"""
