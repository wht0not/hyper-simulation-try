...
DEFAULT_SPLIT_ROOT = "data/debug/split/musique"
DEFAULT_HYPERGRAPHS_ROOT = "data/debug/split/musique/hypergraphs"
DEFAULT_OUTPUT_PATH = "musique_split_time.json"
...
def _evaluate_bucket(
    ...
    for row in tqdm(rows, desc=f"Bucket {bucket_name}", unit="inst"):
        ...
        instance_dir = hypergraphs_root / instance_id
...
def run_musique_split_timing(
    ...
    print("\n" + "=" * 72)
    print("MuSiQue split timing finished")
    print("=" * 72)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Batch timing for MuSiQue split buckets")
