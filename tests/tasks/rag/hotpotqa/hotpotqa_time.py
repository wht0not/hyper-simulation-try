...
DEFAULT_OUTPUT_PATH = "hotpotqa_time.json"
...
def _load_instance_graphs(instance_dir: Path, item: dict[str, Any]) -> tuple[LocalHypergraph | None, list[dict[str, Any]]]:
	...
	if isinstance(context, dict):
		...
		doc_count = len(titles) if isinstance(titles, list) else 0
	...
	for data_path in data_paths:
		...
		if doc_count > 0 and data_idx >= doc_count:
			continue
...
def run_hotpotqa_hyper_simulation_timing(
	...
	for instance_dir in tqdm(instance_dirs, desc="HotpotQA hyper-simulation timing", unit="inst"):
...
def main() -> None:
	parser = argparse.ArgumentParser(description="HotpotQA hyper-simulation timing only")
	...
	run_hotpotqa_hyper_simulation_timing(
