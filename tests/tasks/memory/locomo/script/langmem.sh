...
#   --method langmem \
#   --stage compose \
#   --dataset-path "$LOCOMO_DATA/locomo_langmem_locomo10_rag_retrieved.json" \
#   --output-dir "$LOCOMO_DATA" \
#   --model-name "$MODEL_NAME"
#
# pixi run -e simulation python "$LOCOMO_ROOT/run_experiments.py" \
#   --method langmem \
#   --stage answer \
#   --prepared-path "$LOCOMO_DATA/locomo_langmem_locomo10_rag_prepared.json" \
#   --model-name "$MODEL_NAME" \
#   --answer-batch-size "$ANSWER_BATCH_SIZE"
#
# pixi run -e simulation python "$LOCOMO_ROOT/run_experiments.py" \
#   --method langmem \
#   --stage evaluate \
#   --answers-path "$LOCOMO_DATA/locomo_langmem_locomo10_rag_answers.json" \
#   --model-name "$MODEL_NAME" \
#   --judge-max-workers "$JUDGE_MAX_WORKERS" \
#   --llm-judge-repeat "$LLM_JUDGE_REPEAT"
